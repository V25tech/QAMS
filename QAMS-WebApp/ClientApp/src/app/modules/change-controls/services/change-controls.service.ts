...
export class ChangeControlsService {
  ...
  saveChangeControlRegistration(ccValue: CC_Model) {
    ...
    return this.http.post(this.apiUrl + '/changecontrolregistration/savechangecontrolregistration', JSON.stringify(ccValue), { headers });
  }

  getChangeControlById(changeControlId: number) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.get(this.apiUrl + '/changecontrolregistration/' + changeControlId, { headers });
  }

  getAllChangeControlbyPlant(plantId: number) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post(this.apiUrl + '/changecontrolregistration/GetAllChangeControlbyPlant',"{\"pageNumber\":1,\"pageSize\":100, \"planId\":2}", { headers });
  }
